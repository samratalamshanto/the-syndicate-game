...
export const HumanHand = ({ player, isActive, flash = false, coinRef, onNewGame, onBackToSetup }: Props) => {
  ...
  return (
    <section
      ...
      className={`surface-glass relative grid gap-3 rounded-2xl border px-4 py-3 backdrop-blur-sm transition ${
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-full bg-brass text-night shadow">
            <User size={18} />
          </span>
          <div className="leading-tight">
            <h3 className="font-display text-xl font-black">{t.common.yourHand}</h3>
            <p className="text-app-muted text-[11px] uppercase tracking-widest">{player.name}</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div ref={coinRef} className="flex items-center gap-2 rounded-full border border-brass/60 bg-gradient-to-r from-[#2b1b08] via-[#8a5b16] to-[#f1c15a] px-3 py-1.5 text-night shadow-gold">
            <span className="relative grid h-8 w-8 place-items-center rounded-full border border-white/45 bg-gradient-to-br from-[#fff2a8] via-[#d69b25] to-[#6c3f08] shadow">
